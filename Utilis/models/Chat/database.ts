...
type TableInterface = 'Utilisateur' | 'Groupe' | 'Conversation' | 'Participant' | 'Message' | 'Files' | 'StatutLecture';
...
export const getContentFile = async (fileName: TableInterface) => {
  ...
  const info = await FileSystem.getInfoAsync(FileSystem.documentDirectory + fileName + '.db');
  if (!info.exists) {
    FileSystem.writeAsStringAsync(
      FileSystem.documentDirectory + fileName + '.db',
      fileName === 'Message' ? '{}' : '[]',
      {
  ...
  return await FileSystem.readAsStringAsync(FileSystem.documentDirectory + fileName + '.db', {
    encoding: FileSystem.EncodingType.UTF8,
  });
};
...
export const saveContentFile = async (fileName: TableInterface, content: string) => {
  ...
  await FileSystem.writeAsStringAsync(FileSystem.documentDirectory + fileName + '.db', content, {
    encoding: FileSystem.EncodingType.UTF8,
  });
};
