...
export async function createArrayInstanceFrom({
	...
	if (!itemModelPath) {
		...
		console.log(
			`%c the property=${property} is not a array of model reference`,
			'background: #3455; '
		);
		return;
	}
						...
						Object.defineProperties(itemsInstance, {
							[i]: {
								...
								set: async () => {
									...
									console.log(
										`%c arraData.itemsInstance is a readOnly array property , `,
										'background: #3455; '
									);
								},
							},
	...
	arrayInstance.back = async () => {
		if (currentData && currentData.hasPrevPage) {
			...
			console.log(
				`%cback() == null; backPage = ${paging.page - 1}) ;interval = [ 1 ;  ${
	...
	arrayInstance.next = async () => {
		if (currentData && currentData.hasNextPage) {
			...
			// console.log(`%c CLIENT_ERROR %c wrong action`, 'font-weight: bold; font-size: 14px;color: orange; ', 'font-weight: bold; font-size: 20px;color: red; ');
			console.log(
				`%c next() == null; nextPage =${paging.page + 1}, interval = [ 1 ; ${
					currentData.totalPages
				}  ]`,
				'background: #3455; '
			);
