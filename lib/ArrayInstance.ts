...
export async function createArrayInstanceFrom({
  ...
  if (!itemModelPath) {
    console.log(
      `%c CLIENT_ERROR %c createArrayInstanceFrom'`,
  ...
  SQuery.on('list/' + parentModel + '/' + property + ':' + parentId, async (data: { added: any; removed: any }) => {
    const modifData = {
      added: data.added,
    ...
    return await new Promise((rev) => {
      SQuery.emit(
        ...
        async (res: any) => {
          if (res.error) {
            console.log(
              `%c ERROR_SERVER %c ${itemModelPath}:${'list'}`,
              'font-weight: bold; font-size: 14px;color: orange; ',
              'font-weight: bold; font-size: 20px;color: red; '
            );
  ...
  arrayInstance.back = async () => {
    if (currentData && currentData.hasPrevPage) {
      ...
      console.log(
        `%cback() == null; backPage = ${paging.page - 1}) ;interval = [ 1 ;  ${currentData.totalPages}  ]`,
        'background: #3455; '
      );
    }
  };
  arrayInstance.next = async () => {
    if (currentData && currentData.hasNextPage) {
      ...
      console.log(
        `%c next() == null; nextPage =${paging.page + 1}, interval = [ 1 ; ${currentData.totalPages}  ]`,
        'background: #3455; '
      );
    }
  };
  arrayInstance.page = async (page: number) => {
    if (!page) {
      ...
      console.log(
        `%c page(${page}) == null; page interval = [ 1 ; ${currentData.totalPages}  ]`,
        'background: #3455; '
      );
    }
  };
  ...
  arrayInstance.when = (event: string, listener: listenerSchema, uid?: string) => {
