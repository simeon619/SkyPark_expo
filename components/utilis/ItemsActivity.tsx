...
import { GroupeType, upDownGroup, useGroupActivity } from '../../managementState/server/activityUser/groupActivity';
import SkeletonPlaceholder from 'react-native-skeleton-placeholder';
const ItemsActivity = ({ text = '' }: { text: string }) => {
  const { getGroupe, listGroupe, loadingGroupe } = useGroupActivity();
  console.log('🚀 ~ ItemsActivity ~ loadingGroupe:', loadingGroupe);
  const navigation = useNavigation();
  const filteredGroup = useMemo(
    () => (listGroupe ? listGroupe?.filter((item) => item?.name?.toLowerCase().includes(text.toLowerCase())) : []),
    [text, listGroupe]
  );
      ...
      {loadingGroupe && (
        <SkeletonPlaceholder>
          <SkeletonPlaceholder.Item>
            <SkeletonPlaceholder.Item height={verticalScale(35)} width={'95%'} borderRadius={moderateScale(20)} />
            <SkeletonPlaceholder.Item
              height={25}
              alignSelf="center"
              width={200}
    ...
    >
      ...
      >
        <ShadowImage
          ...
          ratioWidth={100}
          children={
            <Image source={{ uri: HOST + item.poster.imgProfile[0].url }} style={{ width: '100%', height: '100%' }} />
          }
        />
        <Image
