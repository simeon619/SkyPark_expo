...
const Discussion = ({ route, navigation }: NavigationStackProps) => {
	...
	useEffect(() => {
		...
		eventEmitter.on(
			EventMessageType.receiveMessage + user.data.idConversation,
					...
					{item?.newMessage.contenuMessage ? (
						...
						item?.filesData?.map((file, i) => {
							if (getTypeFile(file.extension) === 'image')
								return (
									<ImageRatio uri={file.uri} url={file.url || undefined} key={i} ratio={2.5} />
								);
							if (getTypeFile(file.extension) === 'audio')
								return (
									<InstanceAudio voiceUrl={file.url || undefined} voiceUri={file.uri} key={i} />
								);
						})
					)}
				</View>
